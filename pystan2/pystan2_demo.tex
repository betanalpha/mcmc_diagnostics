...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create a nested list of element names, including indexing information,}
\CommentTok{\# from the specified dimensions.  For example \textasciigrave{}name\_array(\textquotesingle{}x\textquotesingle{}, [2, 3]))\textasciigrave{}}
\CommentTok{\# returns the nested list}
\CommentTok{\# \textgreater{}\textgreater{} [[\textquotesingle{}x[1,1]\textquotesingle{}, \textquotesingle{}x[1,2]\textquotesingle{}, \textquotesingle{}x[1,3]\textquotesingle{}],}
\CommentTok{\# \textgreater{}\textgreater{}  [\textquotesingle{}x[2,1]\textquotesingle{}, \textquotesingle{}x[2,2]\textquotesingle{}, \textquotesingle{}x[2,3]\textquotesingle{}]]}
\CommentTok{\# @ param base Base name.}
\CommentTok{\# @ param dims List of array dimensions.}
...
\KeywordTok{def}\NormalTok{ name\_nested\_list(base, dims, current\_idxs}\OperatorTok{=}\NormalTok{[]):}
\NormalTok{  next\_dim }\OperatorTok{=} \BuiltInTok{len}\NormalTok{(current\_idxs)}
  \ControlFlowTok{if}\NormalTok{ next\_dim }\OperatorTok{==} \BuiltInTok{len}\NormalTok{(dims):}
\NormalTok{    str\_idxs }\OperatorTok{=} \StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{.join([ }\BuiltInTok{str}\NormalTok{(idx }\OperatorTok{+} \DecValTok{1}\NormalTok{) }\ControlFlowTok{for}\NormalTok{ idx }\KeywordTok{in}\NormalTok{ current\_idxs ])}
    \ControlFlowTok{return} \SpecialStringTok{f\textquotesingle{}}\SpecialCharTok{\{}\NormalTok{base}\SpecialCharTok{\}}\SpecialStringTok{[}\SpecialCharTok{\{}\NormalTok{str\_idxs}\SpecialCharTok{\}}\SpecialStringTok{]\textquotesingle{}}
  \ControlFlowTok{else}\NormalTok{:}
    \ControlFlowTok{return}\NormalTok{ [ name\_nested\_list(base, dims, current\_idxs }\OperatorTok{+}\NormalTok{ [d])}
             \ControlFlowTok{for}\NormalTok{ d }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(dims[next\_dim]) ]}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# variables.  Expectand must return a single float, int, or logical}
\CommentTok{\# output.}
\CommentTok{\#}
...
\CommentTok{\# The elements of alt\_arg\_names can also be string arrays of arbitrary}
\CommentTok{\# dimension in which case the individual element values are first}
\CommentTok{\# accessed then formatted into matching numeric arrays before being}
...
\NormalTok{        arg\_vals }\OperatorTok{=}\NormalTok{ [ access\_val(name)}
      ...
      \ControlFlowTok{else}\NormalTok{:}
\NormalTok{        arg\_vals }\OperatorTok{=}\NormalTok{ [ numpy.vectorize(access\_val)(alt\_arg\_names[name])}
                     \ControlFlowTok{if} \BuiltInTok{isinstance}\NormalTok{(alt\_arg\_names[name], numpy.ndarray)}
                     \ControlFlowTok{else}\NormalTok{ access\_val(alt\_arg\_names[name])}
                     \ControlFlowTok{for}\NormalTok{ name }\KeywordTok{in}\NormalTok{ nominal\_arg\_names ]}
...
\begin{verbatim}
Last updated: 2024-09-07T20:26:21.851386-04:00
