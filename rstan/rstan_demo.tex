...
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
    \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is}\NormalTok{.null(ylim)) \{}
\NormalTok{      ylim}\OperatorTok{=}\NormalTok{c(}\BuiltInTok{min}\NormalTok{(lower\_inter), }\BuiltInTok{max}\NormalTok{(}\FloatTok{1.05} \OperatorTok{*}\NormalTok{ upper\_inter))}
\NormalTok{    \}}
...
each other for more direct comparison.
