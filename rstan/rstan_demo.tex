...
\CommentTok{\# information, with column{-}major ordering from the specified dimensions.}
\CommentTok{\# For example \textasciigrave{}elem\_names(\textquotesingle{}x\textquotesingle{}, c(2, 3)))\textasciigrave{} returns}
\CommentTok{\# \textgreater{}\textgreater{}  "x[1,1]" "x[2,1]" "x[1,2]" "x[2,2]" "x[1,3]" "x[2,3]"}
\CommentTok{\#}
\CommentTok{\# @ param base Base name.}
\CommentTok{\# @ param dims Vector of array dimensions.}
\CommentTok{\# @ param current\_idxs Dimensions at current level of recursion.}
\CommentTok{\# @ return Vector of element names in column{-}major order.}
\NormalTok{elem\_names }\OperatorTok{\textless{}{-}}\NormalTok{ function(base, dims, current\_idxs}\OperatorTok{=}\NormalTok{c()) \{}
\NormalTok{  next\_dim }\OperatorTok{\textless{}{-}}\NormalTok{ length(dims) }\OperatorTok{{-}}\NormalTok{ length(current\_idxs)}
  \ControlFlowTok{if}\NormalTok{ (next\_dim }\OperatorTok{==} \DecValTok{0}\NormalTok{) \{}
\NormalTok{    name }\OperatorTok{\textless{}{-}}\NormalTok{ do.call(paste0, lapply(current\_idxs,}
\NormalTok{                                   function(idx)}
\NormalTok{                                   paste0(}\ImportTok{as}\NormalTok{.character(idx), }\StringTok{\textquotesingle{},\textquotesingle{}}\NormalTok{)))}
\NormalTok{    name }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(base, }\StringTok{\textquotesingle{}[\textquotesingle{}}\NormalTok{, gsub(}\StringTok{\textquotesingle{},$\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{, name), }\StringTok{\textquotesingle{}]\textquotesingle{}}\NormalTok{)}
    \ControlFlowTok{return}\NormalTok{(name)}
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{    names }\OperatorTok{\textless{}{-}}\NormalTok{ c()}
    \ControlFlowTok{for}\NormalTok{ (d }\KeywordTok{in} \DecValTok{1}\NormalTok{:dims[next\_dim]) \{}
\NormalTok{      names }\OperatorTok{\textless{}{-}}\NormalTok{ c(names, Recall(base, dims, c(d, current\_idxs)))}
\NormalTok{    \}}
    \ControlFlowTok{return}\NormalTok{(names)}
\NormalTok{  \}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# variables.  Expectand must return a single numeric or logical output.}
\CommentTok{\#}
...
\NormalTok{      \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{        arg\_vals }\OperatorTok{\textless{}{-}}\NormalTok{ vector(}\StringTok{"list"}\NormalTok{, length}\OperatorTok{=}\NormalTok{length(nominal\_arg\_names))}
        \ControlFlowTok{for}\NormalTok{ (n }\KeywordTok{in}\NormalTok{ seq\_along(arg\_vals)) \{}
\NormalTok{          alt\_name }\OperatorTok{\textless{}{-}}\NormalTok{ alt\_arg\_names[[nominal\_arg\_names[n]]]}
\NormalTok{          dims }\OperatorTok{\textless{}{-}}\NormalTok{ dim(alt\_name)}
          \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is}\NormalTok{.null(dims)) \{}
\NormalTok{            arg\_vals[[n]] }\OperatorTok{\textless{}{-}}\NormalTok{ access\_val(alt\_name)}
\NormalTok{          \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{            arg\_vals[[n]] }\OperatorTok{\textless{}{-}} \BuiltInTok{apply}\NormalTok{(alt\_name,}
                                   \DecValTok{1}\NormalTok{:length(dims),}
\NormalTok{                                   access\_val)}
\NormalTok{          \}}
\NormalTok{        \}}
\NormalTok{      \}}
\NormalTok{      pushforward\_vals[c, s] }\OperatorTok{\textless{}{-}} \ImportTok{as}\NormalTok{.numeric(do.call(expectand, arg\_vals))}
\NormalTok{    \}}
\NormalTok{  \}}
