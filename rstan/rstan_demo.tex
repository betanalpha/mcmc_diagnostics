...
\NormalTok{      local\_message }\OperatorTok{\textless{}{-}} 
\NormalTok{        paste0(local\_message,}
\NormalTok{               sprintf(}\StringTok{\textquotesingle{}  Chain }\SpecialCharTok{\%s}\StringTok{: }\SpecialCharTok{\%s}\StringTok{ of }\SpecialCharTok{\%s}\StringTok{ transitions (}\SpecialCharTok{\%.2f\%\%}\StringTok{)}\CharTok{\textbackslash{}n}\StringTok{\textquotesingle{}}\NormalTok{,}
\NormalTok{                       c, n\_tds, S, }\DecValTok{100} \OperatorTok{*}\NormalTok{ n\_tds }\OperatorTok{/}\NormalTok{ S),}
                       \StringTok{\textquotesingle{}           saturated the maximum treedepth \textquotesingle{}}\NormalTok{,}
\NormalTok{               sprintf(}\StringTok{\textquotesingle{}of }\SpecialCharTok{\%s}\StringTok{.}\CharTok{\textbackslash{}n}\StringTok{\textquotesingle{}}\NormalTok{, max\_treedepth))}
\NormalTok{    \}}
...
\NormalTok{      local\_message }\OperatorTok{\textless{}{-}} 
\NormalTok{        paste0(local\_message,}
\NormalTok{               sprintf(}\StringTok{\textquotesingle{}  Chain }\SpecialCharTok{\%s}\StringTok{: Average proxy acceptance \textquotesingle{}}\NormalTok{, c),}
\NormalTok{               sprintf(}\StringTok{\textquotesingle{}statistic (}\SpecialCharTok{\%.3f}\StringTok{)}\CharTok{\textbackslash{}n}\StringTok{\textquotesingle{}}\NormalTok{, ave\_accept\_proxy),}
                       \StringTok{\textquotesingle{}           is smaller than 90}\SpecialCharTok{\% o}\StringTok{f the target \textquotesingle{}}\NormalTok{,}
...
\NormalTok{    desc }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(}\StringTok{\textquotesingle{}All Hamiltonian Monte Carlo diagnostics are \textquotesingle{}}\NormalTok{,}
                   \StringTok{\textquotesingle{}consistent with reliable Markov chain Monte Carlo.\textquotesingle{}}\NormalTok{)}
...
\NormalTok{  transformed\_vals }\OperatorTok{\textless{}{-}} \DecValTok{0}
...
\NormalTok{  \} }\ControlFlowTok{else} \ControlFlowTok{if}\NormalTok{ (t }\OperatorTok{==} \DecValTok{1}\NormalTok{) \{}
    \ControlFlowTok{if}\NormalTok{ (}\BuiltInTok{min}\NormalTok{(expectand\_vals\_list[[name]]) }\OperatorTok{\textless{}=} \DecValTok{0}\NormalTok{) \{}
...
\NormalTok{  \} }\ControlFlowTok{else} \ControlFlowTok{if}\NormalTok{ (t }\OperatorTok{==} \DecValTok{2}\NormalTok{) \{}
    \ControlFlowTok{if}\NormalTok{ (}\BuiltInTok{min}\NormalTok{(expectand\_vals\_list[[name]]) }\OperatorTok{\textless{}=} \DecValTok{0} \OperatorTok{|}
        \BuiltInTok{max}\NormalTok{(expectand\_vals\_list[[name]] }\OperatorTok{\textgreater{}=} \DecValTok{1}\NormalTok{)) \{}
...
\CommentTok{\#                            the second dimension indexes the sequential}
\CommentTok{\#                            states within each Markov chain}
\CommentTok{\# @params B The number of nominal histogram bins}
\NormalTok{plot\_rhats }\OperatorTok{\textless{}{-}}\NormalTok{ function(expectand\_vals\_list, B}\OperatorTok{=}\DecValTok{25}\NormalTok{) \{}
\NormalTok{  validate\_named\_list\_of\_arrays(expectand\_vals\_list,}
                                \StringTok{\textquotesingle{}expectand\_vals\_list\textquotesingle{}}\NormalTok{)}

\NormalTok{  N }\OperatorTok{\textless{}{-}}\NormalTok{ length(expectand\_vals\_list)}
\NormalTok{  rhats }\OperatorTok{\textless{}{-}}\NormalTok{ rep(NA, N)}

\NormalTok{  n }\OperatorTok{\textless{}{-}} \DecValTok{1}
  \ControlFlowTok{for}\NormalTok{ (name }\KeywordTok{in}\NormalTok{ names(expectand\_vals\_list)) \{}
\NormalTok{    expectand\_vals }\OperatorTok{\textless{}{-}}\NormalTok{ expectand\_vals\_list[[name]]}
    \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is}\NormalTok{.null(expectand\_vals)) \{}
...
\NormalTok{    rhat }\OperatorTok{\textless{}{-}}\NormalTok{ compute\_split\_rhat(expectand\_vals)}
    \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is}\NormalTok{.finite(rhat))}
\NormalTok{      rhats[n] }\OperatorTok{\textless{}{-}}\NormalTok{ rhat}
\NormalTok{    n }\OperatorTok{\textless{}{-}}\NormalTok{ n }\OperatorTok{+} \DecValTok{1}
\NormalTok{  \}}

\NormalTok{  val\_min }\OperatorTok{\textless{}{-}} \BuiltInTok{min}\NormalTok{(rhats, na.rm}\OperatorTok{=}\NormalTok{TRUE)}
\NormalTok{  val\_max }\OperatorTok{\textless{}{-}} \BuiltInTok{max}\NormalTok{(rhats, na.rm}\OperatorTok{=}\NormalTok{TRUE)}

\NormalTok{  plot\_min }\OperatorTok{\textless{}{-}} \BuiltInTok{min}\NormalTok{(}\FloatTok{1.1}\NormalTok{, val\_min)}
\NormalTok{  plot\_max }\OperatorTok{\textless{}{-}} \BuiltInTok{max}\NormalTok{(}\FloatTok{1.1}\NormalTok{, val\_max)}
\NormalTok{  B }\OperatorTok{\textless{}{-}}\NormalTok{ B }\OperatorTok{*}\NormalTok{ (plot\_max }\OperatorTok{{-}}\NormalTok{ plot\_min) }\OperatorTok{/}\NormalTok{ (val\_max }\OperatorTok{{-}}\NormalTok{ val\_min)}

\NormalTok{  delta }\OperatorTok{\textless{}{-}}\NormalTok{ (plot\_max }\OperatorTok{{-}}\NormalTok{ plot\_min) }\OperatorTok{/}\NormalTok{ B}
\NormalTok{  plot\_min }\OperatorTok{\textless{}{-}}\NormalTok{ plot\_min }\OperatorTok{{-}}\NormalTok{ delta}
\NormalTok{  plot\_max }\OperatorTok{\textless{}{-}}\NormalTok{ plot\_max }\OperatorTok{+}\NormalTok{ delta}
\NormalTok{  bins }\OperatorTok{\textless{}{-}}\NormalTok{ seq(plot\_min, plot\_max, delta)}
\NormalTok{  B }\OperatorTok{\textless{}{-}}\NormalTok{ B }\OperatorTok{+} \DecValTok{2}
...
\NormalTok{  counts }\OperatorTok{\textless{}{-}}\NormalTok{ hist(rhats, breaks}\OperatorTok{=}\NormalTok{bins, plot}\OperatorTok{=}\NormalTok{FALSE)$counts}
\NormalTok{  max\_y }\OperatorTok{\textless{}{-}} \BuiltInTok{max}\NormalTok{(counts)}
\NormalTok{  y }\OperatorTok{\textless{}{-}}\NormalTok{ counts[idx]}

\NormalTok{  par(mfrow}\OperatorTok{=}\NormalTok{c(}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{), mar }\OperatorTok{=}\NormalTok{ c(}\DecValTok{5}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{1}\NormalTok{))}

\NormalTok{  plot(x, y, }\BuiltInTok{type}\OperatorTok{=}\StringTok{"l"}\NormalTok{,  lwd}\OperatorTok{=}\DecValTok{2}\NormalTok{, col}\OperatorTok{=}\NormalTok{c\_dark,}
\NormalTok{       xlim}\OperatorTok{=}\NormalTok{c(plot\_min, plot\_max), xlab}\OperatorTok{=}\StringTok{"Split hat}\SpecialCharTok{\{R\}}\StringTok{s"}\NormalTok{,}
\NormalTok{       ylim}\OperatorTok{=}\NormalTok{c(}\DecValTok{0}\NormalTok{, }\FloatTok{1.05} \OperatorTok{*}\NormalTok{ max\_y), ylab}\OperatorTok{=}\StringTok{""}\NormalTok{, yaxt}\OperatorTok{=}\StringTok{"n"}\NormalTok{)}

\NormalTok{  abline(v}\OperatorTok{=}\FloatTok{1.1}\NormalTok{, lwd}\OperatorTok{=}\DecValTok{2}\NormalTok{, lty}\OperatorTok{=}\DecValTok{3}\NormalTok{, col}\OperatorTok{=}\StringTok{"\#DDDDDD"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  acov\_buff }\OperatorTok{\textless{}{-}}\NormalTok{ Re(Rs)}
  ...
  \ControlFlowTok{if}\NormalTok{ (acov\_buff[}\DecValTok{1}\NormalTok{] }\OperatorTok{==} \DecValTok{0}\NormalTok{)}
...
\NormalTok{    tau\_hat }\OperatorTok{\textless{}{-}}\NormalTok{ compute\_tau\_hat(expectand\_vals[c,])}
    ...
    \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is}\NormalTok{.infinite(tau\_hat)) \{}
\NormalTok{      no\_warning }\OperatorTok{\textless{}{-}}\NormalTok{ FALSE}
\NormalTok{      body }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(}\StringTok{\textquotesingle{}  Chain }\SpecialCharTok{\%s}\StringTok{: The calculation of hat}\SpecialCharTok{\{tau\}}\StringTok{ is \textquotesingle{}}\NormalTok{,}
...
\NormalTok{    \} }\ControlFlowTok{else} \ControlFlowTok{if}\NormalTok{(}\KeywordTok{is}\NormalTok{.nan(tau\_hat)) \{}
\NormalTok{      no\_warning }\OperatorTok{\textless{}{-}}\NormalTok{ FALSE}
\NormalTok{      body }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(}\StringTok{\textquotesingle{}  Chain }\SpecialCharTok{\%s}\StringTok{: The calculation of hat}\SpecialCharTok{\{tau\}}\StringTok{ is \textquotesingle{}}\NormalTok{,}
...
\NormalTok{                                sprintf(body, c, inc\_tau\_hat))}
\NormalTok{      \}}
\NormalTok{    \}}
\NormalTok{  \}}
  \ControlFlowTok{if}\NormalTok{ (no\_warning) \{}
...
\NormalTok{    message }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(message, desc, }\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{\textquotesingle{}}\NormalTok{)}
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
...
\NormalTok{plot\_ess\_hats }\OperatorTok{\textless{}{-}}\NormalTok{ function(expectand\_vals\_list,}
...
\NormalTok{  ev }\OperatorTok{\textless{}{-}}\NormalTok{ expectand\_vals\_list[[}\DecValTok{1}\NormalTok{]]}
\NormalTok{  validate\_array(ev, }\StringTok{\textquotesingle{}expectand\_vals\_list[[1]]\textquotesingle{}}\NormalTok{)}

\NormalTok{  C }\OperatorTok{\textless{}{-}}\NormalTok{ dim(ev)[}\DecValTok{1}\NormalTok{]}
\NormalTok{  S }\OperatorTok{\textless{}{-}}\NormalTok{ dim(ev)[}\DecValTok{2}\NormalTok{]}
\NormalTok{  ess\_hats }\OperatorTok{\textless{}{-}}\NormalTok{ matrix(NA, C, N)}
...
\NormalTok{    \}}
    ...
    \ControlFlowTok{for}\NormalTok{ (c }\KeywordTok{in} \DecValTok{1}\NormalTok{:C) \{}
\NormalTok{      tau\_hat }\OperatorTok{\textless{}{-}}\NormalTok{ compute\_tau\_hat(expectand\_vals[c,])}
\NormalTok{      ess\_hats[c, n] }\OperatorTok{\textless{}{-}}\NormalTok{ S }\OperatorTok{/}\NormalTok{ tau\_hat}
\NormalTok{    \}}
\NormalTok{    n }\OperatorTok{\textless{}{-}}\NormalTok{ n }\OperatorTok{+} \DecValTok{1}
\NormalTok{  \}}

\NormalTok{  val\_min }\OperatorTok{\textless{}{-}} \BuiltInTok{min}\NormalTok{(ess\_hats, na.rm}\OperatorTok{=}\NormalTok{TRUE)}
\NormalTok{  val\_max }\OperatorTok{\textless{}{-}} \BuiltInTok{max}\NormalTok{(ess\_hats, na.rm}\OperatorTok{=}\NormalTok{TRUE)}

\NormalTok{  plot\_min }\OperatorTok{\textless{}{-}} \BuiltInTok{min}\NormalTok{(min\_ess\_hat, val\_min)}
\NormalTok{  plot\_max }\OperatorTok{\textless{}{-}} \BuiltInTok{max}\NormalTok{(min\_ess\_hat, val\_max)}
\NormalTok{  B }\OperatorTok{\textless{}{-}}\NormalTok{ B }\OperatorTok{*}\NormalTok{ (plot\_max }\OperatorTok{{-}}\NormalTok{ plot\_min) }\OperatorTok{/}\NormalTok{ (val\_max }\OperatorTok{{-}}\NormalTok{ val\_min)}
...
\NormalTok{  max\_y }\OperatorTok{\textless{}{-}}
    \BuiltInTok{max}\NormalTok{(sapply(}\DecValTok{1}\NormalTok{:C,}
\NormalTok{               function(c) }\BuiltInTok{max}\NormalTok{(hist(ess\_hats[c,],}
\NormalTok{                                    breaks}\OperatorTok{=}\NormalTok{bins,}
\NormalTok{                                    plot}\OperatorTok{=}\NormalTok{FALSE)$counts)))}
...
\NormalTok{  colors }\OperatorTok{\textless{}{-}}\NormalTok{ c(c\_dark, c\_mid\_highlight, c\_mid, c\_light\_highlight)}
  ...
  \ControlFlowTok{for}\NormalTok{ (c }\KeywordTok{in} \DecValTok{1}\NormalTok{:C) \{}
\NormalTok{    counts }\OperatorTok{\textless{}{-}}\NormalTok{ hist(ess\_hats[c,], breaks}\OperatorTok{=}\NormalTok{bins, plot}\OperatorTok{=}\NormalTok{FALSE)$counts}
\NormalTok{    y }\OperatorTok{\textless{}{-}}\NormalTok{ counts[idx]}

\NormalTok{    plot(x, y, }\BuiltInTok{type}\OperatorTok{=}\StringTok{"l"}\NormalTok{, main}\OperatorTok{=}\NormalTok{paste(}\StringTok{"Chain"}\NormalTok{, c),}
\NormalTok{         lwd}\OperatorTok{=}\DecValTok{2}\NormalTok{, col}\OperatorTok{=}\NormalTok{colors[c],}
\NormalTok{         xlim}\OperatorTok{=}\NormalTok{c(plot\_min, plot\_max),}
\NormalTok{         xlab}\OperatorTok{=}\StringTok{"Empirical Effective Sample Sizes"}\NormalTok{,}
\NormalTok{         ylim}\OperatorTok{=}\NormalTok{c(}\DecValTok{0}\NormalTok{, }\FloatTok{1.05} \OperatorTok{*}\NormalTok{ max\_y), ylab}\OperatorTok{=}\StringTok{""}\NormalTok{, yaxt}\OperatorTok{=}\StringTok{"n"}\NormalTok{)}

\NormalTok{    abline(v}\OperatorTok{=}\NormalTok{min\_ess\_hat, lwd}\OperatorTok{=}\DecValTok{2}\NormalTok{, lty}\OperatorTok{=}\DecValTok{3}\NormalTok{, col}\OperatorTok{=}\StringTok{"\#DDDDDD"}\NormalTok{)}
\NormalTok{  \}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{    desc }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(}\StringTok{\textquotesingle{}All expectands checked appear to be behaving \textquotesingle{}}\NormalTok{,}
                   ...
                   \StringTok{\textquotesingle{}Carlo estimation.\textquotesingle{}}\NormalTok{)}
\NormalTok{    desc }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(strwrap(desc, max\_width, }\DecValTok{0}\NormalTok{), collapse}\OperatorTok{=}\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{\textquotesingle{}}\NormalTok{)}
\NormalTok{    message }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(message, desc)}
\NormalTok{  \}}
...
\NormalTok{      tau\_hat }\OperatorTok{\textless{}{-}}\NormalTok{ compute\_tau\_hat(expectand\_vals[c,])}
      ...
      \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is}\NormalTok{.infinite(tau\_hat) }\OperatorTok{|}
          \KeywordTok{is}\NormalTok{.nan(tau\_hat)      }\OperatorTok{|}
\NormalTok{          (tau\_hat }\OperatorTok{/}\NormalTok{ S) }\OperatorTok{\textgreater{}} \DecValTok{5}\NormalTok{      ) \{}
\NormalTok{        failed\_names }\OperatorTok{\textless{}{-}}\NormalTok{ c(failed\_names, name)}
...
\NormalTok{      sprintf(}\StringTok{\textquotesingle{}The expectands }\SpecialCharTok{\%s}\StringTok{ triggered diagnostic warnings.\textquotesingle{}}\NormalTok{,}
...
\NormalTok{    desc }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(strwrap(desc, max\_width, }\DecValTok{0}\NormalTok{), collapse}\OperatorTok{=}\StringTok{\textquotesingle{}}\CharTok{\textbackslash{}n}\StringTok{\textquotesingle{}}\NormalTok{)}
\NormalTok{    message }\OperatorTok{\textless{}{-}}\NormalTok{ paste0(message, desc, }\StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{)}
\NormalTok{  \}}
...
\NormalTok{      paste0(sprintf(body,}
\NormalTok{                     paste(failed\_inc\_tau\_hat\_names, collapse}\OperatorTok{=}\StringTok{", "}\NormalTok{)),}
             \StringTok{\textquotesingle{}If the incremental empirical integrated \textquotesingle{}}\NormalTok{,}
             \StringTok{\textquotesingle{}autocorrelation times are unreliable or too large \textquotesingle{}}\NormalTok{,}
             \StringTok{\textquotesingle{}then the Markov chains have not explored long \textquotesingle{}}\NormalTok{,}
             \StringTok{\textquotesingle{}enough for the autocorrelation estimates to be \textquotesingle{}}\NormalTok{,}
             \StringTok{\textquotesingle{}reliable.\textquotesingle{}}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create array of element names from the specified dimensions.}
\CommentTok{\# For example \textasciigrave{}name\_array(\textquotesingle{}x\textquotesingle{}, c(2, 3)))\textasciigrave{} returns the array}
\CommentTok{\# \textgreater{}\textgreater{}      [,1]     [,2]     [,3]}
\CommentTok{\# \textgreater{}\textgreater{} [1,] "x[1,1]" "x[1,2]" "x[1,3]"}
\CommentTok{\# \textgreater{}\textgreater{} [2,] "x[2,1]" "x[2,2]" "x[2,3]"}
...
\NormalTok{name\_array }\OperatorTok{\textless{}{-}}\NormalTok{ function(base, dims) \{}
  \CommentTok{\# Validate inputs}
  \ControlFlowTok{if}\NormalTok{ ( }\OperatorTok{!}\KeywordTok{is}\NormalTok{.character(base) ) \{}
\NormalTok{    stop(paste0(}\StringTok{\textquotesingle{}Input variable \textquotesingle{}}\NormalTok{, base, }\StringTok{\textquotesingle{} is not a character.\textquotesingle{}}\NormalTok{))}
\NormalTok{  \}}

  \ControlFlowTok{if}\NormalTok{ ( }\OperatorTok{!}\KeywordTok{is}\NormalTok{.vector(dims) }\OperatorTok{|} \OperatorTok{!}\KeywordTok{is}\NormalTok{.numeric(dims) ) \{}
\NormalTok{    stop(paste0(}\StringTok{\textquotesingle{}Input variable \textquotesingle{}}\NormalTok{, dims, }\StringTok{\textquotesingle{} is not a numeric vector.\textquotesingle{}}\NormalTok{))}
\NormalTok{  \}}

  \CommentTok{\# Create element names and format them into desired array.}
  \CommentTok{\# Assumes that R arrays are stored in column{-}major order.}
\NormalTok{  names }\OperatorTok{\textless{}{-}}\NormalTok{ elem\_names(base, dims)}
\NormalTok{  array(names, dims)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Evaluate a list of expectands on the values of an arbitrary number of}
\CommentTok{\# input variables.  Expectands must all return a scalar numeric or}
\CommentTok{\# logical output.}
\CommentTok{\#}
\CommentTok{\# By default expectand argument values are accessed by name}
\CommentTok{\# in expectand\_vals\_list.  If a non{-}null alt\_arg\_names is provided then}
\CommentTok{\# the alternate names are used to access values in expectand\_vals\_list.}
\CommentTok{\# The elements of alt\_arg\_names can also be character arrays of}
\CommentTok{\# arbitrary dimension in which case the individual element values are}
\CommentTok{\# first accessed then formatted into matching numeric arrays before}
\CommentTok{\# being passed to the expectand.}
\CommentTok{\#}
\CommentTok{\# @param expectand\_vals\_list A named list of two{-}dimensional arrays.}
...
\CommentTok{\#                            Markov chain.}
\CommentTok{\# @param expectand\_list List of functions with the same arguments.}
\CommentTok{\# @param alt\_arg\_names Optional named list of alternate names for the}
...
\CommentTok{\#         expectand\_list with the same names as expectand\_list.}
\NormalTok{eval\_expectand\_pushforwards }\OperatorTok{\textless{}{-}}\NormalTok{ function(expectand\_vals\_list,}
\NormalTok{                                        expectand\_list,}
\NormalTok{                                        alt\_arg\_names}\OperatorTok{=}\NormalTok{NULL) \{}
  \CommentTok{\# Validate inputs}
\NormalTok{  validate\_named\_list\_of\_arrays(expectand\_vals\_list,}
                                \StringTok{\textquotesingle{}expectand\_vals\_list\textquotesingle{}}\NormalTok{)}

  \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\KeywordTok{is}\NormalTok{.}\BuiltInTok{list}\NormalTok{(expectand\_list)) \{}
\NormalTok{    stop(paste0(}\StringTok{\textquotesingle{}Input variable \textasciigrave{}expectand\_list\textasciigrave{} is not a list.\textquotesingle{}}\NormalTok{))}
\NormalTok{  \}}


  \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\NormalTok{Reduce(}\StringTok{"\&"}\NormalTok{, sapply(expectand\_list, }\KeywordTok{is}\NormalTok{.function))) \{}
\NormalTok{    stop(paste0(}\StringTok{\textquotesingle{}The elements of input variable \textasciigrave{}expectand\_list\textasciigrave{} \textquotesingle{}}\NormalTok{,}
                \StringTok{\textquotesingle{}are not all functions.\textquotesingle{}}\NormalTok{))}
\NormalTok{  \}}

  \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\KeywordTok{is}\NormalTok{.null(alt\_arg\_names)) \{}
...
\NormalTok{  \}}
  ...
  \CommentTok{\# Check consistent of expectand arguments}
\NormalTok{  nominal\_arg\_names }\OperatorTok{\textless{}{-}}\NormalTok{ formalArgs(expectand\_list[[}\DecValTok{1}\NormalTok{]])}

\NormalTok{  compare\_args }\OperatorTok{\textless{}{-}}\NormalTok{ function(e) \{}
\NormalTok{    Reduce(}\StringTok{"\&"}\NormalTok{, formalArgs(e) }\OperatorTok{==}\NormalTok{ nominal\_arg\_names)}
\NormalTok{  \}}
\NormalTok{  arg\_consistency }\OperatorTok{\textless{}{-}}\NormalTok{ Reduce(}\StringTok{"\&"}\NormalTok{, sapply(expectand\_list, compare\_args))}
  \ControlFlowTok{if}\NormalTok{(}\OperatorTok{!}\NormalTok{arg\_consistency) \{}
\NormalTok{    stop(paste0(}\StringTok{\textquotesingle{}The arguments of the functions in \textasciigrave{}expectand\_list\textasciigrave{} \textquotesingle{}}\NormalTok{,}
                \StringTok{\textquotesingle{}are not consistent with each other.\textquotesingle{}}\NormalTok{))}
\NormalTok{  \}}

  \CommentTok{\# Ensure that all argument replacements are arrays}
\NormalTok{  alt\_arg\_names\_array }\OperatorTok{=} \BuiltInTok{list}\NormalTok{()}
  \ControlFlowTok{for}\NormalTok{ (name }\KeywordTok{in}\NormalTok{ names(alt\_arg\_names)) \{}
\NormalTok{    alt\_arg\_names\_array[[name]] }\OperatorTok{\textless{}{-}} \ImportTok{as}\NormalTok{.array(alt\_arg\_names[[name]])}
\NormalTok{  \}}
...
\NormalTok{    check\_arg\_names }\OperatorTok{\textless{}{-}}\NormalTok{ nominal\_arg\_names}
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
...
\NormalTok{    check\_arg\_names }\OperatorTok{\textless{}{-}}\NormalTok{ c(sapply(alt\_arg\_names,}
\NormalTok{                                function(alt) }\ImportTok{as}\NormalTok{.}\BuiltInTok{list}\NormalTok{(alt)),}
\NormalTok{                         recursive}\OperatorTok{=}\NormalTok{TRUE)}
\NormalTok{  \}}

\NormalTok{  missing\_args }\OperatorTok{\textless{}{-}}\NormalTok{ setdiff(check\_arg\_names, names(expectand\_vals\_list))}
  \ControlFlowTok{if}\NormalTok{ (length(missing\_args) }\OperatorTok{==} \DecValTok{1}\NormalTok{) \{}
\NormalTok{    stop(paste0(}\StringTok{\textquotesingle{}The expectand argument \textquotesingle{}}\NormalTok{,}
\NormalTok{                paste(missing\_args, collapse}\OperatorTok{=}\StringTok{", "}\NormalTok{),}
                \StringTok{\textquotesingle{} is not in \textasciigrave{}expectand\_vals\_list\textasciigrave{}.\textquotesingle{}}\NormalTok{))}
...
\NormalTok{                paste(missing\_args, collapse}\OperatorTok{=}\StringTok{", "}\NormalTok{),}
                \StringTok{\textquotesingle{} are not in \textasciigrave{}expectand\_vals\_list\textasciigrave{}.\textquotesingle{}}\NormalTok{))}
\NormalTok{  \}}
...
\NormalTok{  pushforward\_vals }\OperatorTok{\textless{}{-}}\NormalTok{ lapply(expectand\_list,}
\NormalTok{                             function(e) matrix(NA, nrow}\OperatorTok{=}\NormalTok{C, ncol}\OperatorTok{=}\NormalTok{S))}

  \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\KeywordTok{is}\NormalTok{.null(alt\_arg\_names))}
\NormalTok{    alt\_names }\OperatorTok{\textless{}{-}}\NormalTok{ lapply(nominal\_arg\_names,}
\NormalTok{                        function(name) alt\_arg\_names\_array[[name]])}

\NormalTok{  expectand\_vals\_env }\OperatorTok{\textless{}{-}} \ImportTok{as}\NormalTok{.environment(expectand\_vals\_list)}
\NormalTok{  access\_val }\OperatorTok{\textless{}{-}}\NormalTok{ function(name) \{}
...
\NormalTok{  \}}
  ...
  \ControlFlowTok{for}\NormalTok{ (c }\KeywordTok{in} \DecValTok{1}\NormalTok{:C) \{}
    \ControlFlowTok{for}\NormalTok{ (s }\KeywordTok{in} \DecValTok{1}\NormalTok{:S) \{}
      \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{is}\NormalTok{.null(alt\_arg\_names)) \{}
\NormalTok{        arg\_vals }\OperatorTok{\textless{}{-}}\NormalTok{ lapply(nominal\_arg\_names, access\_val)}
\NormalTok{      \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{        arg\_vals }\OperatorTok{\textless{}{-}}\NormalTok{ vector(}\StringTok{"list"}\NormalTok{, length}\OperatorTok{=}\NormalTok{length(alt\_names))}
        \ControlFlowTok{for}\NormalTok{ (n }\KeywordTok{in}\NormalTok{ seq\_along(arg\_vals)) \{}
\NormalTok{          arg\_vals[[n]] }\OperatorTok{\textless{}{-}} \BuiltInTok{apply}\NormalTok{(alt\_names[[n]],}
\NormalTok{                                 seq\_along(dim(alt\_names[[n]])),}
\NormalTok{                                 access\_val)}
\NormalTok{        \}}
\NormalTok{      \}}
      \ControlFlowTok{for}\NormalTok{ (i }\KeywordTok{in}\NormalTok{ seq\_along(pushforward\_vals)) \{}
\NormalTok{        pushforward\_vals[[i]][c, s] }\OperatorTok{\textless{}{-}}
          \ImportTok{as}\NormalTok{.numeric(do.call(expectand\_list[[i]], arg\_vals))}
\NormalTok{      \}}
\NormalTok{    \}}
\NormalTok{  \}}
...
\CommentTok{\# variables.  Expectand must return a scalar numeric or logical output.}
\CommentTok{\#}
\CommentTok{\# By default expectand argument values are accessed by name}
\CommentTok{\# in expectand\_vals\_list.  If a non{-}null alt\_arg\_names is provided then}
\CommentTok{\# the alternate names are used to access values in expectand\_vals\_list.}
...
\CommentTok{\#                            indexes the sequential states within each}
\CommentTok{\#                            Markov chain.}
\CommentTok{\# @param expectand Function with arbitrary number of scalar and array}
...
\NormalTok{                                       expectand,}
\NormalTok{                                       alt\_arg\_names}\OperatorTok{=}\NormalTok{NULL) \{}
\NormalTok{  pushforward\_vals }\OperatorTok{\textless{}{-}}\NormalTok{ eval\_expectand\_pushforwards(expectand\_vals\_list,}
                                                  \BuiltInTok{list}\NormalTok{(expectand),}
\NormalTok{                                                  alt\_arg\_names)}
\NormalTok{  pushforward\_vals[[}\DecValTok{1}\NormalTok{]]}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{implicit\_subset\_prob }\OperatorTok{\textless{}{-}}\NormalTok{ function(expectand\_vals\_list,}
\NormalTok{                                 indicator,}
\NormalTok{                                 alt\_arg\_names) \{}
  \CommentTok{\# Evaluate indicator function}
\NormalTok{  indicator\_samples }\OperatorTok{\textless{}{-}}\NormalTok{ eval\_expectand\_pushforward(expectand\_vals\_list,}
\NormalTok{                                                  indicator,}
\NormalTok{                                                  alt\_arg\_names)}

  \CommentTok{\# Verify outputs}
\NormalTok{  unique\_vals }\OperatorTok{\textless{}{-}}\NormalTok{ unique(c(indicator\_samples, recursive}\OperatorTok{=}\NormalTok{TRUE))}

\NormalTok{  valid\_outputs }\OperatorTok{=}\NormalTok{ TRUE}
  \ControlFlowTok{if}\NormalTok{ (length(unique\_vals) }\OperatorTok{==} \DecValTok{1}\NormalTok{) \{}
    \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\NormalTok{(unique\_vals }\OperatorTok{==} \DecValTok{0} \OperatorTok{|}\NormalTok{ unique\_vals }\OperatorTok{==} \DecValTok{1}\NormalTok{))}
\NormalTok{      valid\_outputs }\OperatorTok{=}\NormalTok{ FALSE}
\NormalTok{  \} }\ControlFlowTok{else} \ControlFlowTok{if}\NormalTok{ (length(unique\_vals) }\OperatorTok{==} \DecValTok{2}\NormalTok{) \{}
    \ControlFlowTok{if}\NormalTok{ ( }\OperatorTok{!}\NormalTok{(  Reduce(}\StringTok{"\&"}\NormalTok{, unique\_vals }\OperatorTok{==}\NormalTok{ c(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{))}
           \OperatorTok{|}\NormalTok{ Reduce(}\StringTok{"\&"}\NormalTok{, unique\_vals }\OperatorTok{==}\NormalTok{ c(}\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{))) )}
\NormalTok{      valid\_outputs }\OperatorTok{=}\NormalTok{ FALSE}
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{    valid\_outputs }\OperatorTok{=}\NormalTok{ FALSE}
\NormalTok{  \}}
  \ControlFlowTok{if}\NormalTok{(}\OperatorTok{!}\NormalTok{valid\_outputs)}
\NormalTok{    stop(paste0(}\StringTok{\textquotesingle{}The function \textasciigrave{}indicator\textasciigrave{} must return only \textquotesingle{}}\NormalTok{,}
...
\NormalTok{  ensemble\_mcmc\_est(indicator\_samples)[}\DecValTok{1}\NormalTok{:}\DecValTok{2}\NormalTok{]}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{      min\_f }\OperatorTok{\textless{}{-}} \BuiltInTok{min}\NormalTok{(min\_f, baseline)}
\NormalTok{      max\_f }\OperatorTok{\textless{}{-}} \BuiltInTok{max}\NormalTok{(max\_f, baseline)}
\NormalTok{    \}}
...
\NormalTok{      delta }\OperatorTok{\textless{}{-}} \FloatTok{0.1}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{util}\SpecialCharTok{$}\FunctionTok{plot\_rhats}\NormalTok{(samples)}
\end{Highlighting}
\end{Shaded}

\includegraphics{rstan_demo_files/figure-pdf/unnamed-chunk-17-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{util}\SpecialCharTok{$}\FunctionTok{plot\_ess\_hats}\NormalTok{(samples)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{rstan_demo_files/figure-pdf/unnamed-chunk-26-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{euclidean\_length }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(x) \{}
  \FunctionTok{sqrt}\NormalTok{(}\FunctionTok{sum}\NormalTok{(x}\SpecialCharTok{**}\DecValTok{2}\NormalTok{))}
\NormalTok{\}}

\NormalTok{beta\_names }\OtherTok{\textless{}{-}}\NormalTok{ util}\SpecialCharTok{$}\FunctionTok{name\_array}\NormalTok{(}\StringTok{\textquotesingle{}beta\textquotesingle{}}\NormalTok{, }\FunctionTok{c}\NormalTok{(data}\SpecialCharTok{$}\NormalTok{M))}
...
\NormalTok{  util}\SpecialCharTok{$}\FunctionTok{eval\_expectand\_pushforward}\NormalTok{(samples, euclidean\_length,}
                                  \FunctionTok{list}\NormalTok{(}\StringTok{\textquotesingle{}x\textquotesingle{}}\OtherTok{=}\NormalTok{beta\_names))}
...
\includegraphics{rstan_demo_files/figure-pdf/unnamed-chunk-27-1.pdf}
...
\NormalTok{pi\_est }\OtherTok{\textless{}{-}}\NormalTok{ util}\SpecialCharTok{$}\FunctionTok{implicit\_subset\_prob}\NormalTok{(samples,}
                                    ...
                                    \FunctionTok{list}\NormalTok{(}\StringTok{\textquotesingle{}x\textquotesingle{}}\OtherTok{=}\StringTok{\textquotesingle{}beta[2]\textquotesingle{}}\NormalTok{))}

\FunctionTok{cat}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(}\StringTok{"Posterior probability that beta[2] is greater than zero = "}\NormalTok{,}
           \FunctionTok{sprintf}\NormalTok{(}\StringTok{"\%.2f +/{-} \%.2f."}\NormalTok{, pi\_est[}\DecValTok{1}\NormalTok{], }\DecValTok{2} \SpecialCharTok{*}\NormalTok{ pi\_est[}\DecValTok{2}\NormalTok{])))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Posterior probability that beta[2] is greater than zero = 0.13 +/- 0.11.
\end{verbatim}
