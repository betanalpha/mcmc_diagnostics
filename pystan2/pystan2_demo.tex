...
\CommentTok{\# variables.  Variable non{-}keyword arguments in the expectand function}
\CommentTok{\# are supported by passing a alt\_arg\_names instance with a varargs\_name}
\CommentTok{\# element containing a list of variable names.}
...
\NormalTok{                               varargs\_name}\OperatorTok{=}\StringTok{\textquotesingle{}varargs\textquotesingle{}}\NormalTok{):}
  \CommentTok{"""Evaluate an expectand on the values of an arbitrary number}
\CommentTok{     of input variables."""}
  \CommentTok{\# Validate inputs}
\NormalTok{  validate\_dict\_of\_arrays(expectand\_vals\_dict, }\StringTok{\textquotesingle{}expectand\_vals\_dict\textquotesingle{}}\NormalTok{)}

  \ControlFlowTok{if} \KeywordTok{not} \BuiltInTok{callable}\NormalTok{(expectand):}
    \ControlFlowTok{raise} \PreprocessorTok{TypeError}\NormalTok{(}\StringTok{\textquotesingle{}Input variable \textasciigrave{}expectand\textasciigrave{} is not a \textquotesingle{}}
                    \StringTok{\textquotesingle{}callable function.\textquotesingle{}}\NormalTok{)}

  \CommentTok{\# Check existence of all expectand arguments}
\NormalTok{  spec }\OperatorTok{=}\NormalTok{ inspect.getfullargspec(expectand)}
\NormalTok{  nominal\_arg\_names }\OperatorTok{=}\NormalTok{ spec.args}
\NormalTok{  variable\_arg\_names }\OperatorTok{=}\NormalTok{ spec.varargs}

  \ControlFlowTok{if}\NormalTok{ alt\_arg\_names }\KeywordTok{is} \VariableTok{None}\NormalTok{:}
    \ControlFlowTok{if}\NormalTok{ variable\_arg\_names }\KeywordTok{is} \KeywordTok{not} \VariableTok{None}\NormalTok{:}
      \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\StringTok{\textquotesingle{}An expectand with variable non{-}keyword \textquotesingle{}}
                       \StringTok{\textquotesingle{}arguments requires an explicit alt\_arg\_names \textquotesingle{}}
                       \StringTok{\textquotesingle{}argument containing a \textasciigrave{}varargs\_name\textasciigrave{} key.\textquotesingle{}}\NormalTok{)}
\NormalTok{    arg\_names }\OperatorTok{=}\NormalTok{ nominal\_arg\_names}
  \ControlFlowTok{else}\NormalTok{:}
...
\NormalTok{    arg\_names }\OperatorTok{=}\NormalTok{ [ alt\_arg\_names[nom] }\ControlFlowTok{for}\NormalTok{ nom }\KeywordTok{in}\NormalTok{ nominal\_arg\_names ]}

    \ControlFlowTok{if}\NormalTok{ variable\_arg\_names }\KeywordTok{is} \KeywordTok{not} \VariableTok{None}\NormalTok{:}
      \CommentTok{\# Validate variable non{-}keyword arguments replacements}
      \ControlFlowTok{if}\NormalTok{ varargs\_name }\KeywordTok{not} \KeywordTok{in}\NormalTok{ alt\_arg\_names.keys():}
        \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{( }\StringTok{\textquotesingle{}The variable non{-}keyword argument \textquotesingle{}}
                         \SpecialStringTok{f\textquotesingle{}}\SpecialCharTok{\{}\NormalTok{varargs\_name}\SpecialCharTok{\}}\SpecialStringTok{ does not have a \textquotesingle{}}
                          \StringTok{\textquotesingle{}replacement in \textasciigrave{}alt\_arg\_names\textasciigrave{}.\textquotesingle{}}\NormalTok{)}

      \ControlFlowTok{if} \KeywordTok{not}\NormalTok{ (}\BuiltInTok{isinstance}\NormalTok{(alt\_arg\_names[varargs\_name], }\BuiltInTok{list}\NormalTok{)}
              \KeywordTok{and} \BuiltInTok{all}\NormalTok{([ }\BuiltInTok{isinstance}\NormalTok{(a, }\BuiltInTok{str}\NormalTok{)}
                      \ControlFlowTok{for}\NormalTok{ a }\KeywordTok{in}\NormalTok{ alt\_arg\_names[varargs\_name] ])  ):}
        \ControlFlowTok{raise} \PreprocessorTok{TypeError}\NormalTok{( }\SpecialStringTok{f\textquotesingle{}alt\_arg\_names[}\SpecialCharTok{\{}\NormalTok{varargs\_name}\SpecialCharTok{\}}\SpecialStringTok{] is \textquotesingle{}}
                          \StringTok{\textquotesingle{}not a list of strings.\textquotesingle{}}\NormalTok{)}

      \CommentTok{\# Append variable non{-}keyword arguments}
\NormalTok{      arg\_names }\OperatorTok{+=}\NormalTok{ alt\_arg\_names[varargs\_name]}

\NormalTok{  missing\_args }\OperatorTok{=} \BuiltInTok{set}\NormalTok{(arg\_names).difference(expectand\_vals\_dict.keys())}
  \ControlFlowTok{if} \BuiltInTok{len}\NormalTok{(missing\_args) }\OperatorTok{==} \DecValTok{1}\NormalTok{:}
...
\NormalTok{  pushforward\_vals }\OperatorTok{=}\NormalTok{ numpy.zeros([C, S])}
  \ControlFlowTok{for}\NormalTok{ c }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(C):}
    \ControlFlowTok{for}\NormalTok{ s }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(S):}
\NormalTok{      arg\_vals }\OperatorTok{=}\NormalTok{ [ expectand\_vals\_dict[name][c, s]}
                   \ControlFlowTok{for}\NormalTok{ name }\KeywordTok{in}\NormalTok{ arg\_names ]}
\NormalTok{      pushforward\_vals[c, s] }\OperatorTok{=}\NormalTok{ expectand(}\OperatorTok{*}\NormalTok{arg\_vals)}
...
\KeywordTok{def}\NormalTok{ ensemble\_mcmc\_quantile\_est(expectand\_vals, probs):}
  \CommentTok{\# Validate inputs}
\NormalTok{  util.validate\_array(expectand\_vals, }\StringTok{\textquotesingle{}expectand\_vals\textquotesingle{}}\NormalTok{)}
...
\includegraphics{pystan2_demo_files/figure-pdf/cell-18-output-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{def}\NormalTok{ euclidean\_length(x1, x2, x3):}
  \ControlFlowTok{return}\NormalTok{ math.sqrt(x1}\OperatorTok{**}\DecValTok{2} \OperatorTok{+}\NormalTok{ x2}\OperatorTok{**}\DecValTok{2} \OperatorTok{+}\NormalTok{ x3}\OperatorTok{**}\DecValTok{2}\NormalTok{)}
...
\NormalTok{                                  \{}\StringTok{\textquotesingle{}x1\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}beta[1]\textquotesingle{}}\NormalTok{,}
                                   \StringTok{\textquotesingle{}x2\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}beta[2]\textquotesingle{}}\NormalTok{,}
                                   \StringTok{\textquotesingle{}x3\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}beta[3]\textquotesingle{}}\NormalTok{\})}
...
\begin{verbatim}
Last updated: 2024-09-05T14:57:36.394903-04:00
