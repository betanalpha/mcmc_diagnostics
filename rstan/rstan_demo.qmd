...
we can flag a constant expectand by looking at its empirical variance.
...
{{< include _includes/plot_rhats.qmd >}}
...
{{< include _includes/plot_ess_hats.qmd >}}
...
are encountering problems; even this compact summary is overwhelming.

Visualizing the split $\hat{R}$ and empirical effective sample sizes
with histograms is a bit more manageable.

```{r}
util$plot_rhats(samples)
```

```{r}
util$plot_ess_hats(samples)
```

That said, we can also avoid overwhelming ourselves by focus on just
...
cat(paste0("Posterior probability that beta[2] is greater than zero = ",
           sprintf("%.2f +/- %.2f.", pi_est[1], 2 * pi_est[2])))
```
