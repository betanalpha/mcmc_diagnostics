...
```{.r include="stan_utility_rstan.R" start-line=17 end-line=29}
```

```{.r include="stan_utility_rstan.R" start-line=31 end-line=49}
```
...
```{.r include="stan_utility_rstan.R" start-line=51 end-line=192}
```
...
```{.r include="stan_utility_rstan.R" start-line=194 end-line=239}
```

```{r}
plot_inv_metric <- function(adaptation_info, B=25) {
  adaptation_info <- get_adaptation_info(fit)
...
```{.r include="stan_utility_rstan.R" start-line=241 end-line=261}
```
...
```{.r include="stan_utility_rstan.R" start-line=263 end-line=302}
```
...
```{.r include="stan_utility_rstan.R" start-line=304 end-line=325}
```
...
```{.r include="stan_utility_rstan.R" start-line=327 end-line=515}
```
...
```{.r include="stan_utility_rstan.R" start-line=517 end-line=661}
```
...
```{.r include="stan_utility_rstan.R" start-line=663 end-line=720}
```
...
```{.r include="stan_utility_rstan.R" start-line=722 end-line=833}
```
...
```{.r include="stan_utility_rstan.R" start-line=835 end-line=892}
```
...
```{.r include="stan_utility_rstan.R" start-line=894 end-line=976}
```
...
```{.r include="stan_utility_rstan.R" start-line=978 end-line=1162}
```

**something about summarizing**

```{.r include="stan_utility_rstan.R" start-line=1164 end-line=1323}
```
...
```{.r include="stan_utility_rstan.R" start-line=1509 end-line=1597}
```
...
```{.r include="stan_utility_rstan.R" start-line=1599 end-line=1657}
```
...
```{.r include="stan_utility_rstan.R" start-line=1659 end-line=1739}
```
...
```{.r include="stan_utility_rstan.R" start-line=1741 end-line=1824}
```
...
```{r}
diagnostics <- extract_hmc_diagnostics(fit)
check_all_hmc_diagnostics(diagnostics)
```
...
```{r}
plot_inv_metric(get_adaptation_info(fit), 75)
```
...
```{r}
display_stepsizes(diagnostics)
```

```{r}
display_ave_accept_proxy(diagnostics)
```
...
```{r}
samples <- extract_expectands(fit)

plot_div_pairs(samples, diagnostics, 
...
```{r}
summarize_expectand_diagnostics(samples)
```
...
```{r}
clip_output(capture.output(check_all_expectand_diagnostics(samples)), 
            27, 21)
```
...
```{r}
check_all_expectand_diagnostics(samples[1:4])
```
...
plot_empirical_correlogram(samples[["alpha"]], 300,
                           rho_lim=c(-0.05, 1.05), "alpha")
plot_empirical_correlogram(samples[["beta[1]"]], 300,
                           rho_lim=c(-0.05, 1.05), "beta[1]")
plot_empirical_correlogram(samples[["beta[2]"]], 300,
                           rho_lim=c(-0.05, 1.05), "beta[2]")
plot_empirical_correlogram(samples[["beta[3]"]], 300,
...
length, leaving only the equivalent of about four independent samples across 
each chain.
...
```{r}
plot_chain_sep_pairs(samples[["alpha"]], "alpha", 
...
expectand we plot split $\hat{R}$ against the largest empirical effective 
sample size amongst the four Markov chains.

```{r}
rhats <- compute_split_rhats(samples)
max_eesss <- compute_max_eesss(samples)

par(mfrow=c(1, 1), mar = c(5, 5, 2, 1))
plot(rhats, max_eesss,
     col=c_dark, pch=16, cex=0.8,
     xlab="Split Rhat", xlim=c(0.95, 2),
     ylab="Empirical Effective\nSample Size", ylim=c(5, 100))
```

Every expectand with a large split $\hat{R}$s also exhibits a particularly
small maximum empirical effective sample size, confirming that the latter 
...
plot_expectand_pushforward(samples[["alpha"]], 25, display_name="alpha")
plot_expectand_pushforward(samples[["beta[1]"]], 25, display_name="beta[1]")
plot_expectand_pushforward(samples[["beta[1]"]], 25, display_name="beta[2]")
plot_expectand_pushforward(samples[["beta[1]"]], 25, display_name="beta[3]")
```
...
```{r}
par(mfrow=c(2, 2), mar = c(5, 4, 2, 1))
plot_expectand_pushforward(samples[["alpha"]], 10, display_name="alpha")
plot_expectand_pushforward(samples[["beta[1]"]], 10, display_name="beta[1]")
plot_expectand_pushforward(samples[["beta[1]"]], 10, display_name="beta[2]")
plot_expectand_pushforward(samples[["beta[1]"]], 10, display_name="beta[3]")
```
